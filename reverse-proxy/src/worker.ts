...
process.on("message", (message: string) => {
  const workerMessage: WorkerMessageType = JSON.parse(message);
  ...
  const req = protocol.request(options, (res) => {
    ...
    res.on("end", () => {
      logger.info(
        `${workerMessage.id} - Response for ${options.method} ${options.hostname} ${options.path} using ${serverInfo.protocol}: ${res.statusCode} - ${responseBody}`
      );

      try {
        responseBody = JSON.parse(responseBody);
      } catch {}
      const responseMessage: WorkerResponseType = {
  ...
  req.on("error", (err: Error) => {
    ...
    logger.error(
      `${workerMessage.id} - Error for ${options.method} ${options.path}: ${err.message}`
    );
    if (process.send) {
      process.send(JSON.stringify(responseMessage));
    }
